...
- **Credits are real**: grants and spends persist to the Butterbase `accounts`
  table (updates go through `PATCH /accounts/:id`, the only single-row update
  route the Data API exposes), so the displayed balance genuinely decrements
  across requests and survives restarts. If a write ever fails, the responder
  logs it and falls back to an in-memory balance so the demo never hard-stops.
